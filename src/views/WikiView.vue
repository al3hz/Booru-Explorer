      ...
      <aside class="wiki-sidebar">
        <div class="sidebar-search">
          ...
          <div class="search-input-wrapper">
            ...
            />
            <button
              class="wiki-search-btn"
              @click="submitWikiSearch(null)"
              title="Search"
            >
            ...
            <div class="accordion-content">
              <ul class="recent-list">
                <li v-for="page in recentChanges.slice(0, 20)" :key="page.id">
                  <a
                    @click.prevent="submitWikiSearch(page.title)"
                    href="#"
                    :title="page.title"
                  >
          ...
          <div v-else-if="currentView" class="wiki-content" key="content">
            ...
            <header class="wiki-header">
              ...
              <div class="title-section">
                ...
                <span
                  v-if="currentView.type === 'artist' && artist"
                  class="status-badge"
                  :class="{
                    active: !artist.is_deleted && !artist.is_banned,
                    inactive: artist.is_deleted || artist.is_banned,
                  }"
                >
              ...
              <div
                class="wiki-aliases"
                v-if="currentView.otherNames && currentView.otherNames.length"
              >
                <span
                  v-for="name in currentView.otherNames"
                  :key="name"
                  class="wiki-tag"
                  @click="handleAliasClick(name)"
                >
                  {{ name }}
                </span>
              </div>
              ...
              <div class="links-grid" v-if="artistUrls.length > 0">
                ...
                >
                  <span class="link-icon"
                    ><i :class="getLinkIconClass(url.url)"></i
                  ></span>
            ...
            <div class="wiki-section" v-if="currentView.body">
              <h3 v-if="currentView.type === 'artist'">
            ...
            <!-- Post Previews -->
            <div
              class="previews-section"
              v-if="
                currentView.previewPosts && currentView.previewPosts.length > 0
              "
            >
            ...
            <div class="actions">
              <button
                v-if="currentView.hasPosts"
                @click="goToPostSearch"
                class="search-posts-btn"
              >
                Search Posts
                <i class="lni lni-search-alt" style="margin-left: 5px"></i>
              </button>
              <a
                :href="currentView.danbooruUrl"
                target="_blank"
                class="danbooru-link"
              >
                View on Danbooru
                <i class="lni lni-link" style="margin-left: 5px"></i>
              </a>
            </div>
          </div>
        </transition>
  ...
  setup() {
    ...
    const handleWikiSearchInput = async () => {
      if (wikiSearchQuery.value.length >= 2) {
        autocompleteResults.value = await fetchWikiAutocomplete(
          wikiSearchQuery.value,
        );
    ...
    const fetchPreviewPosts = async (tag) => {
      try {
        ...
        const searchTags = normalizedTag
          ? `${normalizedTag} -status:deleted`
          : "-status:deleted";
    ...
    const hydrateWikiPosts = async () => {
      const container =
        wikiTextContainer.value ||
        document.querySelector(".wiki-text") ||
        document;
      ...
      if (postStubs.length > 0) {
        ...
        for (let i = 0; i < uniqueIds.length; i += chunkSize) {
          ...
          try {
                ...
                if (post) {
                  ...
                  if (previewUrl) {
                    ...
                    if (stub.parentElement) {
                      ...
                      if (
                        stub.parentElement.parentElement &&
                        stub.parentElement.parentElement.classList.contains(
                          "dtext-list-item",
                        )
                      ) {
                        stub.parentElement.parentElement.classList.add(
                          "gallery-grid-item",
                        );
                      }
            ...
            console.error("[WikiView] Post hydration failed", e);
          }
        }
      }
      ...
      if (assetStubs.length > 0) {
        ...
        for (let i = 0; i < uniqueIds.length; i += chunkSize) {
          ...
          try {
            ...
            assetStubs.forEach((stub) => {
              ...
              if (chunk.includes(aid)) {
                ...
                if (asset) {
                  let previewUrl = "";
                  if (asset.variants) {
                    const pref = asset.variants.find(
                      (v) =>
                        v.type === "360x360" ||
                        v.type === "180x180" ||
                        v.type === "720x720",
                    ...
                    if (asset.post_id) {
                      img.onclick = async () => {
                        if (!wikiInlinePosts.value[asset.post_id]) {
                          try {
                            const post = await DanbooruService.getPost(
                              asset.post_id,
                            );
            ...
            console.error("[WikiView] Asset hydration failed", e);
          }
        }
      }
    };
      ...
      try {
        ...
        currentView.value = {
          ...
          danbooruUrl: wikiId
            ...
            : isArtist
              ? `https://danbooru.donmai.us/artists/${artist.value.id}`
        ...
        if (isArtist) {
          ...
          try {
            ...
            console.warn("Tag check failed", e);
          }
        }
      } catch (e) {
    ...
    const currentIndex = computed(() =>
      !selectedPost.value || posts.value.length === 0
        ? -1
        : posts.value.findIndex((p) => p.id === selectedPost.value.id),
    );
    ...
    const handleNext = () => {
      if (canGoNext.value)
        selectedPost.value = posts.value[currentIndex.value + 1];
    };
    const handlePrev = () => {
      if (canGoPrev.value)
        selectedPost.value = posts.value[currentIndex.value - 1];
    ...
    const handleWikiTextClick = (e) => {
      ...
      if (e.target.classList.contains("dtext-anchor")) {
        ...
        const target =
          ...
          document.getElementById("dtext-" + anchorId);
        if (target) {
          const parentAccordion = target.closest(".dtext-accordion");
          if (parentAccordion) {
            const checkbox = parentAccordion.querySelector(
              'input[type="checkbox"]',
            );
      ...
      if (
        e.target.classList.contains("accordion-checkbox") &&
        e.type === "change"
      ) {
        if (e.target.checked) {
          ...
          if (container) {
            ...
            allCheckboxes.forEach((cb) => {
              if (cb !== e.target && cb.checked) cb.checked = false;
            });
          }
        }
      }
    ...
    const getHeaderIcon = (type) =>
      type === "artist" ? "lni lni-brush-alt" : "lni lni-files";
    ...
    return {
      ...
      goToPostSearch: () => {
        if (currentView.value && currentView.value.searchTag) {
          router.push({
            name: "home",
            query: { tags: currentView.value.searchTag },
          });
        }
      },
    };
...
.wiki-text
  :deep(.dtext-accordion input:not(:checked) ~ .accordion-content > div) {
  visibility: hidden;
