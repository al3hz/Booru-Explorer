    ...
    <div class="artist-modal" @click.stop>
      ...
      <div v-else-if="currentView" class="artist-content">
        ...
        <div class="info-section" v-if="currentView.type === 'artist'">
          ...
          <div v-else class="no-links">No social links found.</div>
        </div>
        ...
        <div
          ...
          v-if="currentView.previewPosts && currentView.previewPosts.length > 0"
        >
          ...
          <div class="preview-grid">
            <div
              ...
              @click.stop="
                $emit('view-post', { post, context: currentView.previewPosts })
              "
        ...
        <div class="actions">
          <button
            class="search-btn"
            @click="$emit('search', currentView.searchTag)"
          >
  ...
  setup(props) {
    ...
    const fetchPreviewPosts = async (tag) => {
      try {
        ...
        const posts = await DanbooruService.getPosts(normalizedTag, 20);
    ...
    const fetchArtistInfo = async () => {
      ...
      try {
        // Fetch Artist Data
        const artistData = await DanbooruService.getArtistByName(
          props.artistName,
        );
        ...
        // Fetch URLs via DanbooruService
    ...
    const fetchWikiPage = async (title) => {
      loading.value = true;
      try {
        const pages = await DanbooruService.getWikiPages(title, 1, 1);
    ...
    const getLinkIconClass = (url) => {
      ...
      if (url.includes("patreon.com") || url.includes("fanbox"))
        return "lni lni-coin";
