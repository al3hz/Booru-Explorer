<template>
  <div class="modal-backdrop">
    ...
    <transition name="fade">
      ...
      >
        <button class="fullscreen-close" @click.stop="toggleMobileFullscreen">
          ×
        </button>
        ...
        <transition :name="transitionName" mode="out-in">
          ...
          <img
            ...
            class="fullscreen-image"
            :style="{
              transform: `scale(${imageScale}) translate(${translateX / imageScale}px, ${translateY / imageScale}px)`,
            }"
            alt="Fullscreen view"
          />
        </transition>
      ...
      <div class="modal-content" ref="modalContent">
        <div class="main-column">
          ...
          >
            ...
            <transition name="fade">
              <div
                v-if="post.is_pending && showPendingOverlay"
              ...
              >
                ...
                <a
                  href="#"
                  @click.prevent="goToWiki('about:mod_queue')"
                  class="overlay-link"
                  title="Read about moderation queue"
                  >Learn more</a
                >
                <button
                  class="overlay-close"
                  @click.stop="showPendingOverlay = false"
                >
                  ×
                </button>
              </div>
            ...
            <div class="image-wrapper" ref="imageContainer">
              ...
              <div
                v-if="
                  notesEnabled && scaledNotes.length > 0 && !isVideo && !isFlash
                "
                class="notes-overlay"
              >
                <div
              ...
              <transition name="slide-up">
                <div v-if="hoveredNote" class="note-bottom-sheet" @click.stop>
                  <div class="sheet-header">
                    ...
                    <button class="sheet-close" @click="hoveredNote = null">
                      ×
                    </button>
                  </div>
          ...
          <div class="info-header">
            ...
            <div class="header-actions">
              ...
              >
                <i
                  v-if="downloading"
                  class="lni lni-spinner lni-is-spinning"
                ></i>
              ...
              >
                <i
                  class="lni"
                  :class="linkCopied ? 'lni-checkmark' : 'lni-link'"
                ></i>
              </button>
              ...
              <div class="reverse-search-dropdown">
                ...
                <transition name="dropdown-fade">
                  <div v-if="searchDropdownOpen" class="search-dropdown-menu">
                    ...
                    >
                      <img
                        src="https://saucenao.com/favicon.ico"
                        alt="SauceNAO"
                        class="service-icon"
                      />
                      <span>SauceNAO</span>
                    ...
                    >
                      <img
                        src="https://iqdb.org/favicon.ico"
                        alt="IQDB"
                        class="service-icon"
                      />
                      <span>IQDB</span>
          ...
          <div class="scrollable-info">
            ...
            <div class="stats-grid">
              <div class="stat-item">
                ...
                <span class="value"
                  >{{ post.image_width }}x{{ post.image_height }}</span
                >
              </div>
              <div class="stat-item">
              ...
              <div class="stat-item">
                ...
                <a
                  :href="`https://danbooru.donmai.us/users/${post.uploader_id}`"
                  target="_blank"
                  class="value source-link"
                  >User #{{ post.uploader_id }}</a
                >
              </div>
            ...
            <div class="commentary-wrapper">
              <transition name="fade-slide">
                <div
                  ...
                  key="loading"
                  class="commentary-section skeleton"
                >
                ...
                <div
                  v-else-if="commentary"
                  key="content"
                  class="commentary-section"
                >
                  <h3>Artist Commentary</h3>
                  <div class="commentary-content">
                    <h4 v-if="commentary.original_title">
                      {{ commentary.original_title }}
                    </h4>
                    <div
                      class="commentary-body"
                      v-html="parseDText(commentary.original_description)"
                    ></div>
                  </div>
                </div>
              </transition>
            ...
            <div class="tags-section">
              ...
              <div v-if="artistTags.length" class="tag-group">
                <h3>Artists</h3>
                <div class="tag-list">
                  <div v-for="tag in artistTags" :key="tag" class="tag-wrapper">
                    <span
                      class="tag artist"
            ...
            <div v-if="comments.length > 0" class="comments-section">
              ...
              >
                ...
                >
                  ...
                  <div
                    class="comment-body"
                    v-html="parseDText(comment.body)"
                  ></div>
                </div>
              </transition-group>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { data: commentaryData, isLoading: commentaryLoading } =
      usePostCommentary(postId);
    const {
      data: familyData,
    ...
    const getImageDisplayDimensions = () => {
      ...
      if (
        !img ||
        !container ||
        !props.post.image_width ||
        !props.post.image_height ||
        cWidth === 0 ||
        cHeight === 0
      ) {
        return {
    ...
    const searchSauceNAO = () => {
      const imageUrl = encodeURIComponent(
        mediaSource.value || props.post.preview_file_url,
      );
      window.open(
        `https://saucenao.com/search.php?url=${imageUrl}`,
        "_blank",
        "noopener,noreferrer",
      );
    ...
    const copyrightTags = computed(() =>
      splitTags(props.post.tag_string_copyright),
    );
    const characterTags = computed(() =>
      splitTags(props.post.tag_string_character),
    );
    const generalTags = computed(() =>
      splitTags(props.post.tag_string_general),
    );
    ...
    const isVideo = computed(() => {
      ...
      if (
        ext === "zip" &&
        props.post.large_file_url &&
        props.post.large_file_url.endsWith(".webm")
      ) {
        return true;
      }
      return false;
    ...
    const loadRuffle = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (window.RufflePlayer) {
          ...
          ruffleContainer.value.innerHTML =
            '<div style="color:white">Player script missing.</div>';
          loading.value = false;
        }
      });
    };
...
/* DText Styles */
.comment-text :deep(.dtext-quote),
.commentary-content :deep(.dtext-quote) {
...
.comment-text :deep(.dtext-code),
.commentary-content :deep(.dtext-code) {
  background: #000;
  padding: 10px;
  border-radius: 4px;
  overflow-x: auto;
...
.comment-text :deep(.dtext-link),
.commentary-content :deep(.dtext-link) {
  color: #38bdf8;
  text-decoration: none;
}

.comment-text :deep(.dtext-link):hover,
.commentary-content :deep(.dtext-link):hover {
  text-decoration: underline;
}

.comment-text :deep(strong),
.commentary-content :deep(strong) {
  color: #fff;
}
.comment-text :deep(em),
.commentary-content :deep(em) {
  color: #ddd;
}
