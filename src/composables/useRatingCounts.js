...
async function fetchTotalCount(tags) {
  try {
    if (isSingleTagQuery(tags)) {
      ...
      const res = await fetch(`${API_BASE}?url=tags.json&search[name]=${encodeURIComponent(tagName)}`);
      ...
      const params = new URLSearchParams({ tags: totalQuery, url: 'counts/posts.json' });
...
async function fetchRatingCount(query) {
  ...
  try {
    const params = new URLSearchParams({ tags: query, url: 'counts/posts.json' });
    const res = await fetch(`${API_BASE}?${params.toString()}`);
